...
export function AdminSidebar({ className, isCollapsed = false, onClose }: AdminSidebarProps) {
                ...
                <div className={cn("flex items-center gap-3", isCollapsed ? "justify-center" : "")}>
                    <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center flex-shrink-0">
                        {user?.avatar ? (
                            ...
                            <span className="text-orange-600 font-bold text-lg">{user?.name?.charAt(0) || "A"}</span>
                        )}
                    </div>
                    {!isCollapsed && (
            ...
            <nav className="flex-1 p-4 overflow-y-auto">
                <ul className="space-y-1">
                    {sidebarItems.map((item) => {
                        ...
                        // Handle root admin path specifically
                        const isExactActive = pathname === item.href && item.href === "/admin"
                        const isChildActive = pathname.startsWith(item.href) && item.href !== "/admin"
                        ...
                        return (
                            <li key={item.name}>
                                ...
                                >
                                    <item.icon className={cn("w-5 h-5 flex-shrink-0", active ? "stroke-[2.5px]" : "")} />
